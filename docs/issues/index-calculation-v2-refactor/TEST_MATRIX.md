...
|--------|---------|--------|------|----------|--------|
| API-IC-001-UT | 服務初始化測試 | P0 | 🔄 待實作 | - | - |
| API-IC-002-UT | 快取鍵生成測試 | P0 | 🔄 待實作 | - | - |
| API-IC-003-UT | 快取TTL過期測試 | P1 | 🔄 待實作 | - | - |
| API-IC-004-UT | 快取LRU淘汰測試 | P1 | 🔄 待實作 | - | - |
| API-IC-005-UT | 相似度計算整合測試 | P2 | 🔄 待實作 | - | - |
| API-IC-006-UT | Sigmoid轉換參數一致性測試 | P1 | 🔄 待實作 | - | - |
| API-IC-007-UT | 關鍵字覆蓋分析測試 | P0 | 🔄 待實作 | - | - |
| API-IC-008-UT | TinyMCE HTML清理測試 | P1 | 🔄 待實作 | - | - |
| API-IC-009-UT | TaskGroup並行執行測試 | P0 | 🔄 待實作 | - | - |
| API-IC-010-UT | TaskGroup錯誤處理測試 | P1 | 🔄 待實作 | - | - |
| API-IC-101-IT | API端點基本功能測試 | P0 | 🔄 待實作 | - | - |
| API-IC-102-IT | 快取行為整合測試 | P0 | 🔄 待實作 | - | - |
| API-IC-103-IT | 輸入驗證測試 | P0 | 🔄 待實作 | - | - |
| API-IC-104-IT | Azure OpenAI服務失敗測試 | P0 | 🔄 待實作 | - | - |
| API-IC-105-IT | 並發請求處理測試 | P1 | 🔄 待實作 | - | - |
| API-IC-106-IT | 大文檔處理測試 | P1 | 🔄 待實作 | - | - |
| API-IC-107-IT | 服務統計端點測試 | P2 | 🔄 待實作 | - | - |
| API-IC-108-IT | 跨語言內容測試 | P1 | 🔄 待實作 | - | - |
| API-IC-201-PT | 響應時間基準測試(30秒) | P0 | 🔄 待實作 | - | - |
| API-IC-202-PT | 快取效能測試(30秒) | P0 | 🔄 待實作 | - | - |
| API-IC-203-PT | 高並發負載測試(60秒) | P0 | 🔄 待實作 | - | - |
| API-IC-204-PT | 記憶體使用效率測試(30秒) | P1 | 🔄 待實作 | - | - |
| API-IC-205-PT | 快取大小限制測試(30秒) | P1 | 🔄 待實作 | - | - |
| API-IC-301-E2E | 完整工作流程測試 | P0 | 🔄 待實作 | - | - |
| API-IC-302-E2E | 錯誤恢復測試 | P1 | 🔄 待實作 | - | - |
| API-IC-303-E2E | 監控和日誌整合測試 | P2 | 🔄 待實作 | - | - |
...
|-------------------|---------------------|---------------------|---------------------|------------------|
| 服務初始化        | 1/0                 | 0/0                 | 0/0                 | 1/0              |
| 快取機制          | 3/0                 | 1/0                 | 2/0                 | 6/0              |
| 相似度計算        | 2/0                 | 0/0                 | 0/0                 | 2/0              |
| 關鍵字分析        | 1/0                 | 0/0                 | 0/0                 | 1/0              |
| HTML處理          | 1/0                 | 0/0                 | 0/0                 | 1/0              |
| 並行處理          | 2/0                 | 1/0                 | 1/0                 | 4/0              |
| API端點           | 0/0                 | 5/0                 | 1/0                 | 6/0              |
| 監控統計          | 0/0                 | 1/0                 | 0/0                 | 1/0              |
| 端對端測試        | 0/0                 | 0/0                 | 0/0                 | 3/0              |
|-------------------|---------------------|---------------------|---------------------|------------------|
| **總計**          | **10/0**            | **8/0**             | **5/0**             | **26/0**         |

效能測試詳情:
------------------------------------------------------------
測試案例                    | 平均回應時間  | SLA 狀態
----------------------------|---------------|----------
基準測試 (1KB/1KB)          | XXX ms        | ⚠️ PENDING
中型文檔 (10KB/5KB)         | XXX ms        | ⚠️ PENDING
大型文檔 (30KB/10KB)        | XXX ms        | ⚠️ PENDING
快取命中測試                | XXX ms        | ⚠️ PENDING
高並發測試 (50 QPS)         | XXX ms        | ⚠️ PENDING
----------------------------|---------------|----------
整體平均                    | XXX ms        | ⚠️ PENDING
...
---------------------|---------------|----------
快取命中率           | XX%           | > 60%
快取響應時間 (P50)   | XX ms         | < 50ms
快取響應時間 (P95)   | XX ms         | < 100ms
快取大小             | XX/1000       | < 1000
