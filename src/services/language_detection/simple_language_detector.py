...
class SimplifiedLanguageDetector(LanguageDetectionService):
    ...
    async def detect_language(self, text: str) -> LanguageDetectionResult:
        ...
        try:
            ...
            # 2. First check what langdetect thinks this is
            try:
            ...
            # 4. Check if langdetect detected an unsupported language directly
            if initial_detected_lang not in ["en", "zh-TW", "zh", "zh-cn", "zh-tw"]:
                logger.warning(f"langdetect detected unsupported language: {initial_detected_lang}")
                raise UnsupportedLanguageError(
                    detected_language=initial_detected_lang,
                    supported_languages=["en", "zh-TW"],
                    confidence=initial_confidence,
                    user_specified=False
                )

            # 5. Step 1: Check unsupported language threshold (10% of total)
