...
class ServiceError(Exception):
    ...
    def to_response(self) -> dict:
        ...
        if not self.error_code:
            self.error_code = ErrorCodes.SYSTEM_INTERNAL_ERROR

        if not self.status_code:
            self.status_code = ERROR_CODE_MAPPING.get(self.error_code, {}).get("status_code", 500)

        return {
            "error_code": self.error_code,
            "message": self.message or ERROR_CODE_MAPPING.get(self.error_code, {}).get("message", "Unknown error"),
...
class ExternalServiceError(ServiceError):
    ...
    def __init__(self, message: str | None = None):
        ...
        super().__init__(
            message=message or "External service unavailable",
