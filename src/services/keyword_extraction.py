...
- Caching mechanism for identical text results (100x speed improvement for repeated
  requests)
"""
import asyncio
import hashlib
...
class KeywordExtractionService(BaseService):
    ...
    async def process(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        Supports language detection, language-specific processing, caching, and parallel
        processing.
        """
        ...
        try:
            ...
            if cached_result is not None:
                ...
                cached_result = cached_result.copy()  # Don't modify original cached
                # data
                ...
                self.logger.info(
                    ...
                    f"processing_time={processing_time}ms (cached), "
            ...
            self.logger.info(
                ...
                f"intersection_size="
                f"{result['intersection_stats']['intersection_count']}, "
            ...
            processing_time = int((time.time() - start_time) * 1000)
            self.logger.error(f"Bilingual keyword extraction failed: {e!s}")
            raise
    ...
    async def _extract_keywords_bilingual(
        ...
        try:
            ...
            self.logger.error(f"Failed to get prompt for {language}: {e!s}")
            raise ValueError(f"Prompt not available for language: {language}") from None
        ...
        if self.enable_parallel_processing:
            ...
            round1_task = asyncio.create_task(
                self._extract_single_round(formatted_prompt, round_num=1)
            )
            round2_task = asyncio.create_task(
                self._extract_single_round(formatted_prompt, round_num=2)
            )
            ...
            self.logger.debug(
                f"Parallel round extraction completed in {round_processing_time}ms"
            )
        else:
            # Sequential execution (original behavior)
            round_start_time = time.time()
            round1_keywords = await self._extract_single_round(
                formatted_prompt, round_num=1
            )
            round2_keywords = await self._extract_single_round(
                formatted_prompt, round_num=2
            )
            round_processing_time = int((time.time() - round_start_time) * 1000)

            self.logger.debug(
                f"Sequential round extraction completed in {round_processing_time}ms"
            )
    ...
    async def _extract_single_round(self, prompt: str, round_num: int) -> list[str]:
        ...
        try:
            ...
            self.logger.debug(
                f"Round {round_num}: LLM call took {llm_time:.2f}ms, "
    ...
    def get_service_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "multilingual_standardizer_stats": (
                self.multilingual_standardizer.get_all_standardization_stats()
            ),
...
def get_keyword_extraction_service(
    ...
    Args:
        ...
        use_gpt41_mini: Use GPT-4.1 mini Japan East for better performance (default:
            True)
