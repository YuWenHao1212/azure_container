...
from src.services.exceptions import PromptNotAvailableError, UnsupportedLanguageError
...
class TestKeywordExtractionLanguageUnit:
    ...
    async def test_service_cleanup_on_error(self, keyword_service,
        ...
        with (
            pytest.raises(AzureOpenAIError),
            patch.object(keyword_service.unified_prompt_service, 'get_prompt_with_config') as mock_get_prompt
        ):
                mock_get_prompt.return_value = (
                    "English prompt template",
                    Mock(temperature=0.3, max_tokens=500, top_p=1.0, seed=42,
