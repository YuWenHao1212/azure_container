...
class PromptValidator:
    ...
    def validate_all_prompts(self) -> bool:
        ...
        for prompt_type, files in files_by_type.items():
            ...
            versions_map = self._check_duplicate_versions(files, prompt_type)
    ...
    def _validate_prompt_file(self, file_path: Path, prompt_type: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.errors.append(f"{relative_path}: Invalid YAML syntax - {str(e)}")
                return False
            ...
            # Validate version
            if 'version' in data:
                if not self._validate_version(data['version'], relative_path):
                    valid = False
            ...
            self.errors.append(f"{relative_path}: Unexpected error during validation - {str(e)}")
            return False
    ...
    def _validate_llm_config(self, llm_config: Any, file_path: Path) -> bool:
        ...
        if 'seed' in llm_config:
            if not isinstance(llm_config['seed'], int):
...
def validate_json_files():
    ...
    for file_path in json_files:
        ...
        try:
            ...
            logger.error(f"  ✗ {relative_path}: Invalid JSON - {str(e)}")
            all_valid = False
        except Exception as e:
            logger.error(f"  ✗ {relative_path}: Error reading file - {str(e)}")
            all_valid = False
