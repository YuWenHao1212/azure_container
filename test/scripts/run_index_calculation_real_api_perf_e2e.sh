...
# Real API Test Runner for Index Calculation V2
...
if [ "$SHOW_HELP" = true ]; then
    echo "Index Calculation Real API Test Runner (Performance + E2E)"
...
log_environment_info() {
    ...
    log_message "Script Version: Index Calculation Real API Test Runner v1.0"
...
cleanup_old_logs() {
    local test_id="$1"
    local log_pattern="$LOG_DIR/test_suite_index_calculation_real_api_perf_e2e_*_${test_id}.log"
...
run_test() {
    ...
    local test_output_file="$LOG_DIR/test_suite_index_calculation_real_api_perf_e2e_${TIMESTAMP}_${test_id}.log"
    ...
    # Execute test with real API configuration
...
run_performance_tests() {
    ...
    if [ -n "$SPECIFIC_PERF_TESTS" ]; then
        ...
        for test_id in "${TEST_IDS[@]}"; do
            ...
            case "$test_id" in
                "p50"|"P50"|"benchmark")
                    test_id="API-IC-201-PT"
                    ;;
                "cache")
                    test_id="API-IC-202-PT"
                    ;;
                "concurrency"|"load")
            ...
            for test_entry in "${performance_tests[@]}"; do
                if [[ "$test_entry" == "$test_id:"* ]]; then
                    ...
                    # Use longer timeout for concurrency test
                    local timeout=180
                    if [[ "$test_id" == "API-IC-203-PT" ]]; then
                        timeout=300  # 5 minutes for concurrency test
                    fi
        ...
        for test_entry in "${performance_tests[@]}"; do
            ...
            local test_path="${test_entry#*:}"
            # Default timeout, longer for concurrency
            local timeout=180
            if [[ "$test_id" == "API-IC-203-PT" ]]; then
                timeout=600
            fi
...
run_e2e_tests() {
    echo -e "${BLUE}Running Index Calculation E2E Tests (3 tests)${NC}"
    echo "Testing file: test/e2e/test_index_calculation_v2_e2e_real_api.py"
...
format_performance_results() {
    echo
    echo "=== Index Calculation 效能測試詳細結果 ==="
    echo
    echo "| 測試編號 | 測試名稱 | P50 (秒) | P95 (秒) | 成功率 | 目標 | 狀態 |"
    echo "|----------|----------|----------|----------|--------|------|------|"
    
    # API-IC-201-PT (Response Time Benchmark)
    local result=$(read_performance_results "API-IC-201-PT")
    IFS='|' read -r p50 p95 success_rate <<< "$result"
    
    local p50_status="✅"
    if [[ " ${FAILED_TESTS[@]} " =~ " API-IC-201-PT " ]]; then
        p50_status="❌"
    elif [ "$p50" != "N/A" ] && (( $(echo "$p50 > 1.0" | bc -l) )); then
        p50_status="❌"
    fi
...
generate_report() {
    ...
    echo
    echo "==============================================="
    echo "Index Calculation Real API 測試報告 (Performance + E2E)"
    echo "==============================================="
    ...
    echo ""
    echo "E2E測試 (3個測試):"
    echo "  - test/e2e/test_index_calculation_v2_e2e_real_api.py"
    echo
    ...
    if [ ${#FAILED_TESTS[@]} -gt 0 ]; then
        ...
        for test_id in "${FAILED_TESTS[@]}"; do
            local priority=$(get_test_priority "$test_id")
            echo "❌ $test_id ($priority) - 日誌: $LOG_DIR/test_suite_index_calculation_real_api_perf_e2e_*_${test_id}.log"
        done
        echo
        echo "修復建議:"
...
main() {
    ...
    # Initialize logging
    log_message "=== Index Calculation Real API Test Suite Started ==="
    log_message "Testing Performance and E2E with real Azure OpenAI APIs"
    
    echo -e "${BLUE}=== Index Calculation Real API Test Suite (Performance + E2E) ===${NC}"
    echo "Timestamp: $(date)"
    ...
    if [ -z "$EMBEDDING_API_KEY" ]; then
        echo -e "  ${RED}❌ Error: EMBEDDING_API_KEY not set in .env${NC}"
        log_message "ERROR: EMBEDDING_API_KEY not set"
        exit 1
    fi
    ...
    echo -e "  Embedding Key: ${GREEN}✓ Embedding key configured${NC}"
    ...
    # Final result
    if [ ${#FAILED_TESTS[@]} -eq 0 ]; then
        log_and_print "${GREEN}🎉 All Index Calculation Real API tests passed!${NC}"
        exit 0
    else
...
create_consolidated_performance_json() {
    local consolidated_file="$LOG_DIR/test_suite_index_calculation_real_api_perf_e2e_${TIMESTAMP}_summary.json"
    
    echo '{' > "$consolidated_file"
    echo '  "test_suite": "Index Calculation V2 Performance",' >> "$consolidated_file"
    ...
    echo '    "total_tests": '$(ls -1 $LOG_DIR/performance_API-IC-*-PT_*.json 2>/dev/null | wc -l | xargs)',' >> "$consolidated_file"
    ...
    local first=true
    for json_file in $(ls -t $LOG_DIR/performance_API-IC-*-PT_*.json 2>/dev/null | head -5); do
        if [ "$first" = true ]; then
            first=false
        else
            echo ',' >> "$consolidated_file"
    ...
    for json_file in $(ls $LOG_DIR/performance_API-IC-*-PT_*.json 2>/dev/null); do
        rm -f "$json_file"
...
if [ "$BACKGROUND_EXEC" = true ]; then
    echo "Running Index Calculation Real API tests in background..."
