...
## 3.8 Index Calculation V2 模組 (IC) - 26個測試案例
...
- **名稱**: embedding 服務失敗時的錯誤處理
- **優先級**: P0
- **類型**: 整合測試
- **需求編號**: REQ-003
- **測試目標**: 驗證 Rate Limit、Internal Error、超時處理
...
- **類型**: 效能測試
- **需求編號**: REQ-003
- **測試目標**: Cache hit < 50ms，Hit rate > 60%
...
- **類型**: 效能測試
- **需求編號**: REQ-003
- **測試目標**: 峰值 < 2GB，GC 後回收 > 50%

#### API-IC-205-PT: 快取大小限制測試（30秒）
- **名稱**: 驗證快取大小限制和 LRU 淘汰效率
- **優先級**: P1
- **類型**: 效能測試
- **需求編號**: REQ-003
- **測試目標**: 快取大小保持限制內，淘汰效能 < 10ms
...
| REQ-003 | 履歷匹配指數 (Index Calculation V2) | API-IC-001~010, 101~108, 201~205, 301~303 | 26個 |
...
| LLM Factory | ✅ 8個 | - | - | - | - | 8個 |
| 匹配指數 V2 | ✅ 10個 | ✅ 8個 | ✅ 5個 | ✅ 3個 | ⏳ | 26個 |
| 差距分析 | ⏳ | ⏳ | ⏳ | ⏳ | ⏳ | 0個 |
| **總計** | **106個** | **24個** | **6個** | **3個** | **0個** | **139個** |
...
| Index Calculation V2 | 10/0  | 8/0      | 5/0*     | 3/0*    | 26/0 |
|-------------------|----------|----------|----------|---------|------|
| **總計**          | **107/0**| **24/0** | **6/0**  | **3/0** | **139/0** |
