...
### 9.1 總體統計
- **總測試案例數**: 113個
- **已實作模組**: 3個（健康檢查、關鍵字提取、LLM Factory）
- **待開發模組**: 5個以上
...
| ---------------------------------- | -------- | -------- | -------- | ------- |
| 健康檢查 (HLT)                     | 9        | 1        | 0        | 10      |
| 關鍵字提取 (KW 101-111, 401-416, 315) | 26       | 1        | 1        | 28      |
| 語言檢測 (KW 211-239, 301-314)     | 29       | 14       | 0        | 43      |
| Prompt管理 (KW 241-264)            | 24       | 0        | 0        | 24      |
| LLM Factory 部署映射 (LLM 501-508) | 8        | 0        | 0        | 8       |
| **總計**                           | **96**   | **16**   | **1**    | **113** |
...
- **TEST_SPEC.md 總計：113個測試案例**

差異原因：
1. 本文檔將語言檢測和Prompt管理器的測試案例進行了更細緻的分類和編號
2. 新增了3個之前未記錄的語言檢測測試（API-KW-237~239）
3. 新增了 test_keyword_extraction_extended.py 的 16 個測試案例（API-KW-401~416）
4. 新增了 test_llm_factory_deployment_mapping.py 的 8 個測試案例（SVC-LLM-501~508）
